  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => {
        ...
        const port = parseInt(process.env.DATABASE_PORT) || configService.get<number>('DATABASE_PORT', 5432);
        console.log(`Connecting to database at ${host}:${port}`);
        return {
          type: 'postgres',
          host: host,
          port: port,
          username: process.env.DATABASE_USERNAME || configService.get('DATABASE_USERNAME', 'postgres'),
          password: process.env.DATABASE_PASSWORD || configService.get('DATABASE_PASSWORD', 'postgres'),
          database: process.env.DATABASE_NAME || configService.get('DATABASE_NAME', 'timedeposits'),
