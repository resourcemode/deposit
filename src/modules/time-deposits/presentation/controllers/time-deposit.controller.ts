...
export class TimeDepositController {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves all time deposits with their current balances, plan types, and withdrawal history",
  })
  @ApiOkResponse({
  ...
  @ApiResponse({
    status: 500,
    description:
      "Internal server error occurred while retrieving time deposits",
  })
  @Get()
  ...
  @ApiOperation({
    ...
    description:
      "Updates the balances of all time deposits based on their plan types and interest calculation strategies. Interest is calculated monthly according to each plan's specific rules.",
  })
  @ApiOkResponse({
