...
describe('TimeDepositService Integration', () => {
  ...
  describe('Integration tests with actual database', () => {
    it('should save and retrieve deposits', async () => {
      ...
      const premiumDeposit = new Deposit(null, 'premium', new Decimal(2000), 60);
      ...
      const foundBasic = allDeposits.find(d => 
        d.planType === 'basic' && 
        d.balance.equals(new Decimal(1000)));
      const foundPremium = allDeposits.find(d => 
        d.planType === 'premium' && 
        d.balance.equals(new Decimal(2000)));
      ...
      const testDeposit = await repository.save(new Deposit(
      ...
      expect(updatedDeposit.balance.greaterThan(new Decimal(1000))).toBeTruthy(); // Just check it increased
      
      // Verify other deposits were also updated
      expect(updatedDeposits.length).toBeGreaterThan(0);
      expect(updatedDeposits.every(d => d.balance instanceof Decimal)).toBe(true);
    });
    ...
    it('should handle withdrawals relationship', async () => {
      ...
      const saved = await repository.save(new Deposit(
        100, 
        PlanType.PREMIUM,
        new Decimal(3000),
        60,
      ...
      expect(depositWithWithdrawals?.withdrawals[0].amount instanceof Decimal).toBe(true);
      expect(depositWithWithdrawals?.withdrawals[0].amount.equals(new Decimal(200))).toBe(true);
    });
